...
export const getCourseByID = (req, res) => {
    ...
    " AS endTime,place,department,instructor,capacity,current_capacity FROM courses WHERE cid = ?";
...
export const getCourseByName = (req, res) => {
  const q = "SELECT cid,name,TIME_FORMAT(startTime, '%H:%i') AS startTime,TIME_FORMAT(endTime, '%H:%i')"+ 
  " AS endTime,place,department,instructor,capacity,current_capacity FROM courses WHERE name = ?";
...
export const getCourseByDepartment = (req, res) => {
  const q = "SELECT cid,name,TIME_FORMAT(startTime, '%H:%i') AS startTime,TIME_FORMAT(endTime, '%H:%i')"+ 
  " AS endTime,place,department,instructor,capacity,current_capacity FROM courses WHERE department = ?";
...
export const selectCourse = (req, res) => {
    ...
    db.query(q, [req.body.uid, req.body.cid], (err, data) => {
      ...
      const q2 = "INSERT INTO enrollment(`uid`,`cid`) VALUES (?)";
