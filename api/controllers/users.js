...
export const updatePassword = (req, res) => {
  try {
    ...
    db.query(q2, [req.body.uid], (err, data) => {
        ...
        if (data[0].password != req.body.currentPassword) return res.status(409).json("Current password not match!");
    });

    if (req.body.newPassword != req.body.newPasswordConfirmation) return res.status(409).json("New password not match!");
